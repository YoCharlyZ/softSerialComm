...
DataPacket dataEnvio = {'<', 0, 256, 65536, 'A', true, 0.00, sizeof(DataPacket), '>'}; // Inicialización explícita con caracteres de control
DataPacket dataRecibido = {'<', 0, 0, 0, ' ', false, 0.00, 0, '>'};   // Inicialización para recepción con caracteres de control
...
void sendSoftSerial() { // Enviar datos mediante SoftwareSerial
  if (millis() >= timeLine0 + intervalo) { // Comprueba si ha pasado el intervalo
    ...
    // Actualiza el campo structSize con el tamaño actual del struct
    dataEnvio.structSize = sizeof(dataEnvio);

    // Envía el struct como un bloque de bytes
    softSerial.write(reinterpret_cast<uint8_t*>(&dataEnvio), sizeof(dataEnvio));
    ...
    Serial.print(F("Fin: ")); Serial.print(dataEnvio.charEnd); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.contador32bits, HEX); Serial.println(F(")"));
    Serial.print(F("Bytes Enviados: ")); Serial.println(dataEnvio.structSize);
    Serial.println(F(" "));
...
void readSoftSerial() { // Leer datos recibidos mediante SoftwareSerial
  if (softSerial.available() >= sizeof(dataRecibido)) { // Comprueba si hay suficientes datos para leer
    ...
    Serial.println(F("Datos recibidos correctamente:"));
      ...
      Serial.print(F("Inicio: ")); Serial.print(dataRecibido.charInit); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.charInit, HEX); Serial.println(F(")"));
      Serial.print(F("8 bits: ")); Serial.print(dataRecibido.contador8bits); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.contador8bits, HEX); Serial.println(F(")"));
      Serial.print(F("16 bits: ")); Serial.print(dataRecibido.contador16bits); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.contador16bits, HEX); Serial.println(F(")"));
      Serial.print(F("32 bits: ")); Serial.print(dataRecibido.contador32bits); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.contador32bits, HEX); Serial.println(F(")"));
      Serial.print(F("Caracter: ")); Serial.print(dataRecibido.caracter); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.caracter, HEX); Serial.println(F(")"));
      Serial.print(F("Estado (bool): ")); Serial.print(dataRecibido.estado ? "true" : "false"); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.estado, HEX); Serial.println(F(")"));
      Serial.print(F("Valor Flotante: ")); Serial.print(dataRecibido.valorFlotante); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.valorFlotante, HEX); Serial.println(F(")"));
      Serial.print(F("Tamaño del paquete: ")); Serial.print(dataRecibido.structSize); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.structSize, HEX); Serial.println(F(")"));
      Serial.print(F("Fin: ")); Serial.print(dataRecibido.charEnd); Serial.print(F(" (Hex: 0x")); Serial.print(dataEnvio.contador32bits, HEX); Serial.println(F(")"));
