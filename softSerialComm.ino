...
void sendSoftSerial() { // Enviar datos por Software Serial
  ...
  if (millis() >= timeLine0 + intervalo) { // Si paso el lapso de tiempo
    ...
    dataEnvio.contador++; // Incrementa el contador y reinicia si excede 4.294.967.295
    if (dataEnvio.contador > 4294967295) {
      dataEnvio.contador = 0; // Reinicia si excede 4.294.967.295
    }
    timeLine0 = millis(); // Actualiza el tiempo anterior
    digitalWrite(LED_BUILTIN, LOW);
  } else {
